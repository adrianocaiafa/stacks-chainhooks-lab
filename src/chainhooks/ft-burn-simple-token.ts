/**
 * Chainhook: FT Burn - simple-token
 * 
 * Registra um chainhook que monitora eventos de burn (queima) do token fungível `simple-token`
 * do contrato SP1RSWVNQ7TW839J8V22E9JBHTW6ZQXSNR67HTZE9.simple-token na mainnet da Stacks.
 * 
 * Este hook captura eventos de resgate ou contração de supply.
 * 
 * Requisitos:
 * - Configurar HIRO_API_KEY no arquivo .env
 * - Ter acesso à API de Chainhooks da Hiro
 * 
 * Referência: https://docs.hiro.so/en/tools/chainhooks/reference/filters#ft-burn
 */
...
try {
  console.log(`Registrando chainhook para monitorar burns FT: ${ASSET_IDENTIFIER}`);
  
  const chainhook = await client.registerChainhook({
    version: '1',
    name: 'ft-burn-simple-token',
    chain: 'stacks',
    network: 'mainnet',
    filters: {
      events: [
  ...
  console.log('Monitorando burns FT do asset:', ASSET_IDENTIFIER);
} catch (error) {
