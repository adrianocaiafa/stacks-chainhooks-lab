/**
 * Chainhook: FT Monitoring - wizard-token
 * 
 * Registra um chainhook que monitora todos os eventos do token fungível `wizard-token`
 * do contrato SP1RSWVNQ7TW839J8V22E9JBHTW6ZQXSNR67HTZE9.wizard-token na mainnet da Stacks.
 * 
 * Este hook monitora:
...
const ASSET_IDENTIFIER = `${DEPLOYER_ADDRESS}.ft::wizard-token`;
...
try {
  ...
  const chainhook = await client.registerChainhook({
    version: '1',
    name: 'ft-monitoring-wizard-token',
    chain: 'stacks',
    network: 'mainnet',
    filters: {
      events: [
