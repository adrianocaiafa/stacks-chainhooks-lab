/**
 * Chainhook: FT Transfer - simple-token
 * 
 * Registra um chainhook que monitora transferências do token fungível `simple-token`
 * do contrato SP1RSWVNQ7TW839J8V22E9JBHTW6ZQXSNR67HTZE9.simple-token na mainnet da Stacks.
 * 
 * Este hook monitora transferências de qualquer carteira para qualquer carteira.
 * 
 * Requisitos:
 * - Configurar HIRO_API_KEY no arquivo .env
 * - Ter acesso à API de Chainhooks da Hiro
 * 
 * Referência: https://docs.hiro.so/en/tools/chainhooks/reference/filters#ft-transfer
 */
...
// Para fungible tokens, geralmente é contract_address.contract_name::token_name
const ASSET_IDENTIFIER = `${CONTRACT_SIMPLE_TOKEN}::simple-token`;

// Registra e habilita o chainhook
try {
  console.log(`Registrando chainhook para monitorar transferências FT: ${ASSET_IDENTIFIER}`);
  
  const chainhook = await client.registerChainhook({
    version: '1',
    name: 'ft-transfer-simple-token',
    chain: 'stacks',
    network: 'mainnet',
    filters: {
      events: [
  ...
  console.log('Monitorando transferências FT do asset:', ASSET_IDENTIFIER);
} catch (error) {
