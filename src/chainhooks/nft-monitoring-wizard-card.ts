/**
 * Chainhook: NFT Monitoring - wizard-card
 * 
 * Registra um chainhook que monitora todos os eventos do NFT `wizard-card`
...
const ASSET_IDENTIFIER = `${DEPLOYER_ADDRESS}.nft::wizard-card`;
...
try {
  ...
  const chainhook = await client.registerChainhook({
    version: '1',
    name: 'nft-monitoring-wizard-card',
    chain: 'stacks',
    network: 'mainnet',
    filters: {
      events: [
